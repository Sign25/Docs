...
## Приложение В: Интеграция с Office Dashboard

### В.1 Агенты Marketing

| agent_id | name | salary_equivalent | fte_coefficient |
|----------|------|-------------------|-----------------|
| marketing_wb | Реклама WB | 60000 | 1.0 |
| marketing_ozon | Реклама Ozon | 60000 | 1.0 |

### В.2 Инициализация репортеров

```python
# app/tasks/marketing/office.py

from app.utils.office_reporter import OfficeReporter

OFFICE_REPORTERS = {
    "wb": OfficeReporter(
        agent_id="marketing_wb",
...
### В.3 Интеграция в bid_cycle

```python
# app/tasks/marketing/bidding.py

from .office import OFFICE_REPORTERS

@shared_task
def run_bid_cycle():
    # Оба агента работают синхронно
    for key, reporter in OFFICE_REPORTERS.items():
        reporter.report_working(f"Bid Cycle - {key.upper()}")
    
    try:
        # ... логика корректировки ставок ...
        
        wb_stats = get_campaign_stats("wb")
        ozon_stats = get_campaign_stats("ozon")
        
        OFFICE_REPORTERS["wb"].report_idle(metrics={
            "campaigns_active": wb_stats["active"],
            "budget_today": wb_stats["spent"]
        })
        
        OFFICE_REPORTERS["ozon"].report_idle(metrics={
            "campaigns_active": ozon_stats["active"],
            "budget_today": ozon_stats["spent"]
        })
        
    except WBApiError as e:
        OFFICE_REPORTERS["wb"].report_error(f"WB API: {e}")
        raise
    except OzonApiError as e:
        OFFICE_REPORTERS["ozon"].report_error(f"Ozon API: {e}")
        raise
```

### В.4 Heartbeat задача

```python
@shared_task(name='marketing.tasks.office_heartbeat')
...
### В.5 Celery Beat Schedule

```python
"marketing-office-heartbeat": {
    "task": "marketing.tasks.office_heartbeat",
    "schedule": 60.0,
    "options": {"queue": "marketing_default"}
},
```
