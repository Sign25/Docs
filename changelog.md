...
- **Правки (Feedback)** v1.0 — система обратной связи: разделы по модулям, запросы с комментариями, архив, icon picker (lucide SVG), роли [(e2ff627)](https://github.com/Sign25/Docs/commit/e2ff627)
- **Коллеги (Colleagues)** v1.0 — DM между сотрудниками + групповые каналы, WebSocket real-time, онлайн-статус, реакции [(e2ff627)](https://github.com/Sign25/Docs/commit/e2ff627)

### Content Factory

- Поиск товаров по артикулу в таблицах + автоподсказки из /api/content/search
- Промпты разбиты по МП: табы WB/Ozon/YM × 7 промптов
- Выбор товаров из склейки: 3 режима (один / выбранные / вся склейка)
- Медиа-менеджер: загрузка/удаление/drag-перестановка фото
- Сертификаты: просмотр/загрузка PDF/удаление/применение к группе
- Шаблоны фото: список категорий, применение к товару
- Видео отображается на карточке товара
- Колонка «Вн. артикул» в таблицах Информация и История
- Склейка кликабельная (переход на товар)
- scrollbar-gutter: stable (антипрыжок)

### Reputation

- Таб «Промпт» для редактирования системного промпта AI
- Время ответа берётся из summary API (без пересчёта)
- Ленивая загрузка по табам (ускорение)
- Обновлён тип DashboardSummary (все поля из API)

### Watcher

- Фильтр товаров с рейтингом < 4.6

### Коллеги / Каналы

- DM между сотрудниками (секция «Коллеги» в сайдбаре)
- Групповые каналы (секция «Каналы» в сайдбаре)
- Модалка «Написать коллеге» со списком пользователей
- Подтверждение скрытия DM (ConfirmDialog)
- Каналы всегда включены без настроек
- Кнопка DM в админке пользователей
- Роли colleagues и channels в настройках доступа

### Прочее

- Анимация сплэша (сердцебиение)
- Текст ошибки подключения обновлён
- db.py: исправлен UnboundLocalError при недоступной БД
