...
description: "UI Design System v1.0 — CSS-переменные, цветовая модель OKLCH, типографика, отступы, скругления, тёмная тема"
mode: "wide"
---
...
Пример: `oklch(0.623 0.214 259.815)` — Knowledge Blue с яркостью 62.3%, насыщенностью 0.214 и оттенком 259.8° (синий).

### Принцип генерации вариантов

Для каждого модульного цвета генерируется `-light` вариант по формуле:

```css
/* Основной: полная насыщенность */
--module-knowledge: oklch(0.623 0.214 259.815);

/* Light: L → 0.95, C → 0.04, H — сохраняется */
--module-knowledge-light: oklch(0.95 0.04 259.815);
```
...
--module-knowledge-light: oklch(0.3 0.06 259.815);
```
