...
| `ratings` | int[] | — | **Фильтр по оценкам.** Повторяемый query-параметр: `?ratings=1&ratings=2`. Одно или несколько значений 1–5 — вернутся только отзывы с этими оценками (`rating IN (...)`, между значениями OR). Не передан → все оценки. Невалидное значение (`0`, `6`, не-число) → `422`. Комбинируется по AND с остальными фильтрами. См. блок «Фильтр по оценкам» ниже |
| `rating` | int[] | — | Deprecated-алиас `ratings` (обратная совместимость). Тоже повторяемый; можно смешивать с `ratings` — значения объединяются |
...
**Фильтр по оценкам (`ratings`):**

Параметр **повторяемый** — чтобы передать несколько оценок, добавь `ratings=` несколько раз в query string (НЕ через запятую, НЕ `ratings[]=`). Применяется как `rating IN (...)`, то есть отзыв попадает в выдачу, если его оценка равна любому из переданных значений. Так же задаётся и диапазон (например «1–3» = перечислить `1`, `2`, `3`).

| Что нужно показать | Запрос |
|---|---|
| Только 1★ | `?ratings=1` |
| 1★ и 2★ | `?ratings=1&ratings=2` |
| Весь негатив 1–4★ | `?ratings=1&ratings=2&ratings=3&ratings=4` |
| Только 5★ | `?ratings=5` |
| Все оценки (по умолчанию) | параметр не передавать |
| 1★ по Wildberries | `?ratings=1&marketplace=wildberries` |
| 1–2★ за период + поиск | `?ratings=1&ratings=2&date_from=2026-06-01&search=брак` |

- Диапазон каждого значения — `1..5`; `0`, `6` или не-число вернут `422` (ошибка валидации).
- Комбинируется с `marketplace`, `status`, `item_type`, `date_from`/`date_to`, `search` по **AND**; не влияет на пагинацию, `total` и сортировку.
- `rating` (в единственном числе) — устаревший алиас, оставлен для обратной совместимости; работает так же и может смешиваться с `ratings`.
...
| `ratings` | int[] | — | **Фильтр по оценкам.** Повторяемый query-параметр: `?ratings=1&ratings=2`. Одно или несколько значений 1–5 — вернутся только отзывы с этими оценками (`rating IN (...)`, между значениями OR). Не передан → все оценки (как раньше). Невалидное значение (`0`, `6`, не-число) → `422`. Комбинируется по AND с остальными фильтрами |
...
Параметр **повторяемый** — чтобы передать несколько оценок, добавь `ratings=` несколько раз в query string (НЕ через запятую). Применяется как `rating IN (...)`, то есть отзыв попадает в выдачу, если его оценка равна любому из переданных значений.
...
- Диапазон каждого значения — `1..5`; `0`, `6` или не-число вернут `422` (ошибка валидации).
- Фильтр не влияет на дедуп (по-прежнему одна строка = последняя попытка на отзыв), на `total`, пагинацию и сортировку.
- Комбинируется с `marketplace`, `date_from`/`date_to`, `search` по **AND**.
