...
description: "Принципы работы и взаимодействия компонентов ADOLF CORE для junior-разработчиков"
mode: "wide"
---
...
graph TB
    subgraph USERS["👥 Пользователи"]
        U1["Менеджер<br/>(браузер)"]
        U2["Директор<br/>(телефон)"]
    end
    
    subgraph VPS["🖥️ Наш сервер (VPS)"]
        NGINX["🚪 Nginx<br/>(охранник)"]
        
        subgraph DOCKER["Docker контейнеры"]
            OWUI["💬 Open WebUI<br/>(чат-интерфейс)"]
            MW["⚙️ Middleware<br/>(мозг системы)"]
            ETL["📄 ETL<br/>(обработка файлов)"]
            CELERY["⏰ Celery<br/>(фоновые задачи)"]
        end
        
        REDIS["📮 Redis<br/>(очередь)"]
        PG["🗄️ PostgreSQL<br/>(база данных)"]
    end
    
    subgraph EXTERNAL["☁️ Внешние сервисы"]
        TWAI["🤖 Timeweb AI<br/>(искусственный интеллект)"]
        TWKB["📚 Timeweb KB<br/>(база знаний)"]
        OPENAI["🧠 OpenAI<br/>(OCR, классификация)"]
    end
...
```mermaid
sequenceDiagram
    participant U as 👤 Менеджер<br/>(бренд: Охана)
    participant MW as ⚙️ Middleware
    participant AI as 🤖 Timeweb AI
    participant KB as 📚 База знаний
    
    U->>MW: "Какие размеры у худи?"
    
    Note over MW: 1. Проверка: кто это?
    MW->>MW: Это Менеджер бренда "Охана"
    
    Note over MW: 2. Модификация запроса
    MW->>MW: Добавить фильтр:<br/>brand_id = "ohana"
    
    MW->>AI: Запрос + фильтры
    AI->>KB: Поиск (только Охана)
    KB-->>AI: Документы Оханы
    AI-->>MW: "Размеры: S, M, L, XL..."
    
    Note over MW: 3. Логирование
    MW->>MW: Записать в audit_log
    
    MW-->>U: "Размеры: S, M, L, XL..."
```
...
flowchart LR
    ...
    subgraph ETL["⚙️ ETL процесс"]
        WATCH["👀 Watchdog<br/>(следит за папкой)"]
        CONV["🔄 Конвертер<br/>(извлекает текст)"]
        CLASS["🏷️ Классификатор<br/>(определяет тип)"]
    end
    
    subgraph OUTPUT["📤 Результат"]
        MD["📄 Markdown<br/>(чистый текст)"]
        KB["📚 База знаний"]
        ARCH["📦 Архив"]
    end
...
sequenceDiagram
    ...
    U->>MW: Загрузить файл
    MW->>R: Задача: обработать файл
    MW-->>U: "Файл принят!" (мгновенно)
    
    Note over R,C: Позже, в фоне...
    
    R->>C: Вот задача
    C->>C: Обработка файла (1 мин)
    C->>U: 🔔 Уведомление: "Файл обработан"
```
...
```mermaid
flowchart LR
    subgraph SYNC["Синхронные операции"]
        S1["Отправить сообщение в чат"]
        S2["Получить список отзывов"]
        S3["Проверить права доступа"]
    end
    
    subgraph ASYNC["Асинхронные операции"]
        A1["Обработать PDF-файл"]
        A2["Отправить уведомление"]
        A3["Очистить старые логи"]
    end
```
...
sequenceDiagram
    ...
    participant T as 🤖 Timeweb AI
    
    B->>N: HTTPS запрос
    Note over N: SSL расшифровка
    N->>O: HTTP запрос
    
    O->>M: POST /v1/chat/completions<br/>{"message": "Какие отзывы..."}
    
    Note over M: Проверка авторизации
    M->>P: SELECT role, brand_id FROM users
    P-->>M: role=manager, brand=ohana
    
    Note over M: Формирование запроса к AI
    M->>T: Запрос + системный промпт<br/>+ фильтры (brand=ohana)
    
    Note over T: AI обрабатывает запрос
    T-->>M: "Сегодня 5 отзывов..."
    
    Note over M: Логирование
    M->>P: INSERT INTO audit_log...
    
    M-->>O: {"response": "Сегодня 5 отзывов..."}
    O-->>N: HTTP ответ
    N-->>B: HTTPS ответ
    
    Note over B: Пользователь видит ответ
```
...
```mermaid
graph TB
    subgraph ROLES["Иерархия ролей"]
        ADMIN["👑 Administrator<br/>(всё)"]
        DIR["📊 Director<br/>(финансы + всё ниже)"]
        SENIOR["👔 Senior Manager<br/>(модерация + всё ниже)"]
        MANAGER["👤 Manager<br/>(отзывы, цены + всё ниже)"]
        STAFF["👶 Staff<br/>(только чат)"]
    end
...
flowchart TB
    subgraph COMPANY["🏢 Компания"]
        subgraph B1["Бренд: Охана Маркет"]
            M1["Менеджер Анна"]
            M2["Менеджер Борис"]
            D1["📄 Документы Оханы"]
            R1["⭐ Отзывы Оханы"]
        ...
        subgraph SENIOR_LEVEL["Senior Manager +"]
            SM["Senior Manager<br/>(видит ВСЕ бренды)"]
        end
    end
...
```mermaid
flowchart TB
    subgraph UPLOAD["1️⃣ Загрузка"]
        U["👤 Менеджер"]
        OWUI["💬 Open WebUI"]
        INBOX["📥 /inbox"]
    end
    
    subgraph PROCESS["2️⃣ Обработка"]
        WATCH["👀 Watchdog<br/>(заметил файл)"]
        DETECT["🔍 Определение типа"]
        CONV["🔄 Конвертация в .md"]
        OCR["👁️ OCR<br/>(если скан)"]
    end
    
    subgraph CLASSIFY["3️⃣ Классификация"]
        AI["🤖 GPT-5 mini"]
        META["📋 Метаданные:<br/>category, access_level, brand_id"]
    end
    
    subgraph MODERATE["4️⃣ Модерация"]
        SENIOR["👔 Senior Manager"]
        APPROVE["✅ Одобрить"]
        REJECT["❌ Отклонить"]
    end
    
    subgraph FINISH["5️⃣ Завершение"]
        KB["📚 База знаний"]
        ARCH["📦 Архив"]
        NOTIFY["🔔 Уведомление"]
    end
...
| `brand_id` | Какой бренд | ohana_market, ohana_kids, all |
...
stateDiagram-v2
    ...
    uploaded --> processing: ETL начал обработку
    processing --> pending: Классификация завершена
    pending --> approved: Senior одобрил ✅
    pending --> rejected: Senior отклонил ❌
    approved --> indexed: Добавлен в KB
    indexed --> [*]
    rejected --> [*]
```
...
flowchart LR
    subgraph SOURCES["📢 Источники событий"]
        REP["⭐ Новый отзыв"]
        DOC["📄 Документ на модерацию"]
        PRICE["💰 Демпинг-алерт"]
        SYS["⚠️ Ошибка системы"]
    end
...
flowchart TB
    subgraph EVENTS["События"]
        E1["⭐ Новый отзыв<br/>(бренд: Охана)"]
        E2["📄 Документ на модерацию"]
        E3["⚠️ Ошибка системы"]
    end
    
    subgraph ROUTER["Маршрутизатор"]
        R["Правила маршрутизации"]
    end
    
    subgraph USERS["Получатели"]
        U1["👤 Менеджер Оханы"]
        U2["👔 Senior Manager"]
        U3["👑 Administrator"]
    end
...
flowchart TB
    subgraph L1["🔒 Слой 1: Сеть"]
        HTTPS["HTTPS (шифрование)"]
        FW["Firewall (только 80, 443)"]
    end
    
    subgraph L2["🔐 Слой 2: Авторизация"]
        LOGIN["Логин + пароль"]
        JWT["JWT токен"]
        SESSION["Сессия"]
    end
    
    subgraph L3["🛡️ Слой 3: Права"]
        ROLE["Проверка роли"]
        BRAND["Проверка бренда"]
        FILTER["Фильтрация данных"]
    end
    
    subgraph L4["📝 Слой 4: Аудит"]
        LOG["Логирование действий"]
        ALERT["Алерты на подозрительное"]
    end
...
1. **Изучить конкретные разделы:**
   - Раздел 1.1: Open WebUI — обзор интерфейса
   - Раздел 1.2: Pipelines — агенты модулей
   - Раздел 2.5: PostgreSQL — схема базы данных
