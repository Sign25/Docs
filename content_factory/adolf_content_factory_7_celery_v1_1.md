...
| `office_heartbeat` | Обновление статуса в Office Dashboard | low |
...
graph TB
    ...
    TASK_PUBLISH & TASK_SYNC --> OFFICE_DB
    TASK_OFFICE --> OFFICE_DB
    CELERY --> REDIS
```
...
def publish_content(
    ...
    # === Office: начало работы ===
    office_reporter.report_working(f"Публикация черновика {draft_id[:8]}...")
    ...
    try:
        ...
        # === Office: обновление задачи ===
        office_reporter.report_working(f"Публикация {draft.sku} на {draft.marketplace}")
        ...
        if result.success:
            ...
            # === Office: успешное завершение ===
            office_reporter.report_idle(metrics={
                "last_published_sku": draft.sku,
                "marketplace": draft.marketplace
            })
        ...
        # === Office: ошибка публикации ===
        office_reporter.report_error(f"{e.code}: {e.message}")
...
### B.1 Агенты Content Factory

| agent_id | name | salary_equivalent | fte_coefficient |
|----------|------|-------------------|-----------------|
| content_descriptions | Генератор описаний | 60000 | 1.0 |
| content_seo | SEO оптимизатор | 60000 | 1.0 |
...
# Репортеры для агентов Content Factory
OFFICE_REPORTERS = {
    "descriptions": OfficeReporter(
        agent_id="content_descriptions",
        department="content_factory",
        name="Генератор описаний",
        salary_equivalent=60000,
...
### B.3 Heartbeat задача

```python
# tasks/office.py

from celery import shared_task
from . import OFFICE_REPORTERS

@shared_task(name='content_factory.tasks.office_heartbeat')
...
### B.4 Celery Beat Schedule

```python
# Добавить в beat_schedule:

"content-factory-office-heartbeat": {
    "task": "content_factory.tasks.office_heartbeat",
    "schedule": 60.0,  # Каждую минуту
    "options": {"queue": "content_low"}
},
```

### B.5 Паттерн использования в задачах

```python
from . import OFFICE_REPORTERS

reporter = OFFICE_REPORTERS["descriptions"]

@shared_task
def some_task():
    try:
        reporter.report_working("Выполнение задачи")
        
        # ... логика задачи ...
        result = do_work()
        
        reporter.report_idle(metrics={"processed": result.count})
        return {"success": True}
