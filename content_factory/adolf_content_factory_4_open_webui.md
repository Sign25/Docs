...
| **История** | `history` | История генераций и публикаций | Навигация |
...
| Балл | Оценка качества контента (цвет: зелёный ≥80, жёлтый ≥50, красный <50) |
...
| Поле | Описание |
|------|----------|
| Маркетплейс | WB, Ozon, Яндекс.Маркет (выбор из доступных) |
...
| Параметр | Описание |
|----------|----------|
| Порог качества | Слайдер 0–100. Товары с оценкой ниже порога попадают в таблицу |
| WB Token | Поле ввода пароля + статус токена (активен/истекает/невалидный) |
| Ozon | Client ID + API Key + статус токена |
| Яндекс.Маркет | API Key + Business ID + статус токена |
...
| `GET` | `/content/approvals/history` | `getApprovalsHistory` | Таб «История» (фильтры: marketplace, source, status) |
