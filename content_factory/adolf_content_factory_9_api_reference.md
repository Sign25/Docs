# ADOLF Content Factory REST API Справочник (v1.8.7)
...
**Swagger UI:** `GET /docs`
**Версия приложения:** `1.8.43` (см. `app/config.py::APP_VERSION`)
...
|--------|------|----------|
| **v1.8.7** | 2026-07-08 | **Стейджинг сертификата — «загрузить серт → выбрать артикулы → опубликовать».** Новый `POST /api/certificates/{id}/stage` сохраняет серт в **черновик** (несвязанный, переживает F5, на МП не уходит); `GET`/`DELETE /api/certificates/{id}/staged` — показать/сбросить. Артикулы при стейджинге **не указываются**: серт привязывается к **выбранным в черновике** артикулам автоматически на `approve` (для этого черновик генерации/ручной черновик создаётся для той же открытой карточки). Раньше серт, загруженный ДО выбора, «падал» на открытую карточку и не публиковался, если approve шёл по другим артикулам. «С текстом или без» — как обычно: approve пишет текст только если он изменился. Старый `POST /api/certificates/{id}` (с `selected_skus`/`draft_id`) остаётся для случая «артикулы уже известны». |
| **v1.8.6.1** | 2026-07-08 | **Качество генерируемого текста.** Описание теперь генерируется **живым связным текстом без markdown** (раньше модель иногда добавляла `**жирный**`/переносы, которые маркетплейс показывает буквально) — усилён промпт + детерминированная пост-обработка срезает markdown. Разделитель в названии приведён к единому виду (ЯМ/Ozon → `/`, WB → пробелы) — раньше скакал между `/` и `,`. Изменение внутреннее: контракт эндпоинтов тот же, меняется лишь формат генерируемых `title`/`description`. |
| **v1.8.6** | 2026-07-08 | **Черновой порядок медиа переживает перезагрузку страницы (F5).** Собранный в модалке порядок/состав фото можно сохранить на бэке ДО подтверждения: `PUT /api/media/{id}/order` (тело `{marketplace, media_order, video_url}`), сбросить — `DELETE /api/media/{id}/order`. После сохранения `GET /api/media/{id}/list` и `GET /api/content/product` отдают именно этот порядок с флагом **`is_staged: true`** — раньше при возврате на страницу показывался старый порядок с маркетплейса. Это **только для отображения**: на маркетплейс по-прежнему публикует `approve` (`media_order` в теле), после чего черновик автоматически сбрасывается. Также: на `approve` медиа наследует единый выбор `selected_skus` из черновика, если `media_selected_skus`/`media_apply_to_group` не заданы (доведено начатое в v1.8.5). |
...
>
> **Черновой порядок (переживает F5).** Собранный порядок/состав можно сохранить на бэке **до** approve — `PUT /api/media/{id}/order`. Тогда `GET /list` и `GET /api/content/product` отдают именно его (флаг `is_staged: true`), и он не теряется при перезагрузке страницы. Это **только для отображения** — на маркетплейс по-прежнему публикует approve (`media_order` в теле). После публикации медиа черновик автоматически сбрасывается.

### `GET /api/media/{external_id}/list`

Получить фото и видео товара для отображения. Если для карточки сохранён черновой порядок (`PUT /order`, ещё не опубликован) — вернётся он (`is_staged: true`); иначе — порядок как на маркетплейсе (`is_staged: false`).
...
| `is_staged` | bool | `true` — отдан **черновой** порядок (сохранён через `PUT /order`, ещё не опубликован); `false` — порядок как на маркетплейсе |

---

### `PUT /api/media/{external_id}/order`

Сохранить **черновой** порядок/состав медиа карточки (кнопка «Сохранить порядок»). Полностью заменяет ранее сохранённый черновик. На маркетплейс **ничего не публикует** — это делает approve. После сохранения `GET /list` и `/api/content/product` отдают этот порядок (`is_staged: true`), пока карточку не подтвердят или не сбросят.

**Path:** `external_id` — SKU товара (int).

**Запрос (`MediaOrderRequest`):**
```json
{
  "marketplace": "ym",
  "media_order": ["https://.../2.jpg", "https://.../1.jpg", "https://.../new.jpg"],
  "video_url": null
}
```
...
| `media_order` | string[] | Нет | Полный список URL фото в нужном порядке (первое = главное). Полностью заменяет черновик |
| `video_url` | string \| null | Нет | URL видео или `null` (null = без видео) |

**Ответ:** `ProductMediaResponse` с `is_staged: true`. `404` — товар не найден.

> **Важно.** Черновик — только для показа и не заменяет публикацию: на approve всё равно нужно передать `media_order` (и `video_url`) в теле `/drafts/{id}/approve`. После успешной публикации медиа черновик по этим карточкам автоматически удаляется, и `GET /list` снова отдаёт состояние маркетплейса.

---

### `DELETE /api/media/{external_id}/order`

Сбросить черновой порядок карточки — вернуться к состоянию маркетплейса.

**Path:** `external_id` (int). **Query:** `marketplace` (required) — `wb` / `ozon` / `ym`.

**Ответ:** `ProductMediaResponse` с актуальными (уже НЕ staged) медиа (`is_staged: false`).
...
> Используйте этот эндпоинт, когда **артикулы уже известны** (передаёте `selected_skus`/`draft_id`/`apply_to_group`). Если серт загружается **раньше** выбора артикулов — см. стейджинг ниже.

---

### `POST /api/certificates/{external_id}/stage` — черновой серт (до выбора артикулов)

Сохранить сертификат в **черновик**: серт держится на сервере **несвязанным** (переживает F5) и **не уходит на маркетплейс**. Артикулы здесь **не указываются** — серт привяжется к **выбранным в черновике** артикулам автоматически при `approve`.

Для потока **«загрузить серт → выбрать артикулы → опубликовать (с текстом или без)»**.

**Path:** `external_id` — открытая карточка (база склейки).
**Form (`multipart/form-data`):** те же поля, что у `POST /api/certificates/{id}`, **кроме** `selected_skus` / `apply_to_group` / `draft_id` (артикулы не передаются). Минимум по МП тот же (Ozon — `file`+`certificate_number`+`issue_date`+`certificate_name`; WB/YM — `certificate_number`).
...
```json
{
  ...
  "external_id": 203873004,
  "marketplace": "ozon",
  "certificate_name": "Сертификат соответствия",
  "certificate_number": "РОСС RU.…",
  "document_type": "certificate",
  "file_url": "https://…/cert.pdf",
  "message": "Сертификат сохранён (черновик). Привяжется к выбранным артикулам при подтверждении."
}
```

**Как публикуется:** выбираешь артикулы и делаешь `generate` (с текстом) **или** `POST /drafts/manual` (без текста) с `sku` = **та же открытая карточка** и `selected_skus: [выбор]`; затем `POST /drafts/{draft_id}/approve` — бэк **сам** привяжет черновой серт к выбранным (`updated_nm_ids`) и опубликует. По серту в `approve` ничего слать не нужно. «С текстом/без» — управляется тем, изменила ли ты `title`/`description` (approve пишет текст только при изменении).

> **Важно:** черновик привязан к **открытой карточке** (`external_id`), поэтому черновик генерации/ручной черновик должен создаваться для **той же** карточки (её `sku`) — иначе `approve` не подхватит серт.

---

### `GET /api/certificates/{external_id}/staged`

Получить черновой (staged) серт открытой карточки — например, чтобы показать его после перезагрузки страницы.

**Path:** `external_id`. **Query:** `marketplace` (default `wb`).

**Ответ:** объект серта с `is_staged: true`; если черновика нет — `{ "is_staged": false, "external_id": …, "marketplace": … }`.

---

### `DELETE /api/certificates/{external_id}/staged`

Сбросить черновой серт открытой карточки (до публикации).

**Path:** `external_id`. **Query:** `marketplace` (default `wb`). **Ответ:** `{ "success": true, "message": "Черновик сертификата удалён" }`.
...
**Стейджинг серта (загрузка до выбора артикулов).** Если серт нужно загрузить **раньше**, чем выбраны артикулы, используйте `POST /api/certificates/{id}/stage` — серт держится в черновике (несвязанный, переживает F5, на МП не уходит) и привязывается к **выбранным в черновике** артикулам автоматически на `approve` (черновик генерации/ручной черновик создаётся для той же открытой карточки). `GET`/`DELETE /api/certificates/{id}/staged` — показать/сбросить черновик. Обычный `POST /api/certificates/{id}` (с `selected_skus`/`draft_id`) — когда артикулы уже известны.
...
| — | POST | `/api/certificates/{external_id}/stage` |
| — | GET | `/api/certificates/{external_id}/staged` |
| — | DELETE | `/api/certificates/{external_id}/staged` |
