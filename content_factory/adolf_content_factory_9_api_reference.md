# ADOLF Content Factory REST API Справочник (v1.8.3)
...
3. Иначе — медиа уходит только на одну карточку черновика.
...
| `certificate_name` | string | Нет | — | Название. Если не задано — подставляется номер |
...
> - **Ozon** — `file` + `certificate_number` + `issue_date` (файл уходит на Ozon как есть; тип берётся из номера).
> - **WB / YM** — `certificate_number` обязателен; `file` **опционален** (PDF — в кабинете МП, по API только номер).
