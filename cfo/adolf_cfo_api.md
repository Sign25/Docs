...
| `revenue` | float | нет | Выручка нетто (после возвратов), ₽. WB: `retail_price_with_disc`, Ozon: `accruals_for_sale` |
...
| `/pnl/category` | `summary` те же, что у `/pnl/marketplace`; в `data` присутствует строка `"category": "Прочее"` |
