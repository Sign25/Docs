...
| `CFO_API_CACHE_TTL` | `21600` | TTL кеша ответов, секунды (по умолчанию 6 ч) |
| `CFO_API_CACHE_MAXSIZE` | `256` | Максимум записей в кеше ответов |
...
Сервер кеширует ответы на уровне SQL-запросов (ключ — хеш SQL+параметры) с TTL `CFO_API_CACHE_TTL` (по умолчанию 6 ч). Кеш — полностью прозрачный, ответы идентичны некешированным. `POST /api/v1/cfo/exclusions` сбрасывает кеш целиком — следующий запрос сразу видит свежие цифры.

Тяжёлые отчёты, тянущие себестоимость из таблиц `1C_*` через `postgres_fdw` (`/pnl/overview`, `/abc`, `/pnl/category`), на «холодную» считаются десятки секунд (`overview` — до ~2.5 мин), а из кеша отдаются за единицы миллисекунд. Поэтому TTL по умолчанию длинный; для надёжности рекомендуется **прогрев**: после каждой синхронизации данных дёргать `GET /api/v1/cfo/dashboard?month=<пред. месяц>` (он населяет кеш всех тяжёлых запросов), чтобы пользователь всегда попадал в тёплый кеш.
...
| GET | `/api/v1/cfo/dashboard` | Сводка для дашборда: 11 отчётов одним вызовом | `dashboard` |
| GET | `/api/v1/cfo/dashboard/kpi` | Верхние KPI дашборда: 7 чисел + ряды (день/месяц/год) | `dashboard` |
...
| GET | `/api/v1/cfo/onec/quantity-by-month` | Количество по артикулам × месяцам (1С, сч. 90.01.1) | `onec` |
| GET | `/api/v1/cfo/onec/quantity-by-month/export` | То же как `.xlsx` | `onec` |
| GET | `/api/v1/cfo/onec/quantity-by-month/articles` | Список артикулов периода для мультиселекта графика | `onec` |
...
### 6.1.1 GET `/api/v1/cfo/dashboard`

Агрегатор для исполнительного дашборда: за **один вызов** возвращает все наборы данных под 13 виджетов. Тег OpenAPI: `dashboard`. Внутри конкурентно вызываются 11 существующих отчётов; **каждый раздел по форме идентичен соответствующему самостоятельному эндпоинту** (те же поля и схемы). Сбойный/медленный раздел отдаётся как `{ "error": "..." }`, не роняя весь ответ.

**Параметры запроса:**

| Параметр | Тип | Дефолт | Описание |
|----------|-----|--------|----------|
| `month` | `YYYY-MM` | предыдущий полный месяц | Отчётный месяц. Из него выводятся текущий и предыдущий месяцы (KPI-дельта), срез на конец месяца и диапазон последних 5 месяцев для трендов. |
| `window` | int 1..365 | `30` | Окно дней оборачиваемости для запасов (`stock`, `abc_turnover`). |
...
```jsonc
{
  ...
  "periods": {
    ...
    "trend":    { "from": "2026-01", "to": "2026-05" },
    "snapshot": "2026-05-31"
  },
  "overview":       { "current": { /* как /pnl/overview */ }, "previous": { /* за пред. месяц */ } },
  "income_expense": { /* как /onec/income-expense */ },
  "operating_pnl":  { /* как /onec/operating-pnl, диапазон trend */ },
  "marketplace":    { /* как /pnl/marketplace, период current */ },
  "category":       { /* как /pnl/category,    период current */ },
  "abc":            { /* как /abc, marketplace=wb, limit=3000 */ },
  "abc_turnover":   { /* как /onec/inventory/abc-turnover, snapshot */ },
  "stock":          { /* как /onec/inventory/stock, топ-30 по стоимости */ },
  "settlements":    { /* как /onec/settlements, топ-30 по экспозиции */ },
  "quantity":       { /* как /onec/quantity-by-month, диапазон trend */ }
}
```

**Производительность.** Ответ собирается из тех же кешируемых SQL-запросов: на тёплом кеше ~0.1 c, на «холодную» — упирается в самый медленный отчёт (`overview`, до ~2.5 мин). Рекомендуется длинный TTL (§4.2) + прогрев `GET /dashboard?month=<пред. месяц>` после каждой синхронизации.

```bash
curl 'http://localhost:8000/api/v1/cfo/dashboard?month=2026-05'
```

| Код | Ситуация |
|:---:|----------|
| 200 | Ответ собран (отдельные разделы могут содержать `{ "error": ... }`) |
| 422 | Неверный `month` / `window` |

---

### 6.1.2 GET `/api/v1/cfo/dashboard/kpi`

Верхний KPI-слой исполнительного дашборда: **7 крупных чисел** за период + для каждого числа временной ряд (`series`) для графика при клике и дельта к предыдущему периоду. Тег OpenAPI: `dashboard`.

Всё считается **консолидированно и с МП** — маркетплейсы на этом дашборде **нигде не исключаются** (в отличие от `/onec/*`). Источники: `finance_pnl` («Доходы и расходы», дневной грейн), `finance_cashflow` (ДДС, дневной), `1C_account_turns_settlements` (дебиторка, помесячно).

**7 чисел (`key`):**

| `key` | Число | `unit` | Расчёт |
|-------|-------|:------:|--------|
| `revenue` | Выручка | `rub` | Статья «Выручка от продаж» (`finance_pnl`). |
| `operating_profit` | Операционная прибыль | `rub` | Валовая (гр. «Продажи») − прочие расходы + прочие доходы. |
| `net_profit` | Чистая прибыль | `rub` | Операционная + «Прочая выручка» − налог на прибыль (25 % операционной; 0 при убытке). |
| `cash_flow` | Денежный поток | `rub` | Σ(Поступления + Списания + Транзитные операции) за период (ДДС). |
| `money_on_accounts` | Деньги на счетах | `rub` | «Конечный остаток» на последний день периода (ДДС). |
| `receivable` | Дебиторская задолженность | `rub` | Σ дебетовых сальдо сч. 60/62 на конец периода (все контрагенты, включая МП). |
| `net_margin` | Рентабельность по чистой прибыли | `pct` | Чистая прибыль / Выручка × 100. |

Потоковые метрики (выручка, прибыль, денежный поток) суммируются за период; остатковые (деньги на счетах, дебиторка) берутся на конец периода. «Прочая выручка» учитывается только в чистой прибыли, не в операционной.

**Параметры запроса:**

| Параметр | Тип | Дефолт | Описание |
|----------|-----|--------|----------|
| `from` | `YYYY-MM-DD` | 1-е число пред. месяца | Начало периода (включительно). |
| `to` | `YYYY-MM-DD` | последний день пред. месяца | Конец периода (включительно). |
| `granularity` | `day` \| `month` \| `year` | `month` | Разбивка ряда `series` за тот же период. |

`from`/`to` передаются только вместе; по умолчанию (ничего не передано) — предыдущий полный календарный месяц. Дельта `delta_pct` считается к предыдущему эквивалентному периоду (для целого календарного месяца — к прошлому календарному месяцу).
  ...
  "kpis": [
    {
      ...
      "unit": "rub",
      "value": 75871730.19,          // за выбранный период
      "previous": 59238896.53,       // предыдущий эквивалентный период
      "delta_pct": 28.08,            // относительный %; для net_margin — п.п.
      "series_granularity": "month", // фактический грейн ряда (у receivable всегда "month")
      "series": [
        { "period": "2026-05", "value": 75871730.19 }
      ]
    }
    // …ещё 6 чисел в порядке revenue, operating_profit, net_profit,
    //   cash_flow, money_on_accounts, receivable, net_margin
  ]
}
```

**Замечания.**
- `delta_pct`: для денежных чисел — относительный % к предыдущему периоду; для `net_margin` (`unit=pct`) — разница в **процентных пунктах**. `null`, если сравнить не с чем.
- Дебиторка — из помесячного регистра, поэтому её `series` всегда помесячный (`series_granularity="month"`) даже при `granularity=day`. У остальных чисел `series_granularity` совпадает с запрошенным.
- Каждое число строит `series` из своего источника (день без данных по метрике в её ряд не попадает; `null`-точки не создаются).

```bash
curl 'http://localhost:8000/api/v1/cfo/dashboard/kpi?from=2026-05-01&to=2026-05-31'
curl 'http://localhost:8000/api/v1/cfo/dashboard/kpi?from=2026-05-01&to=2026-05-31&granularity=day'
```

| Код | Ситуация |
|:---:|----------|
| 200 | Успешно |
| 422 | `from`/`to` переданы порознь, `from > to`, неверная дата или `granularity` не из `day\|month\|year` |
...
| `from` / `to` | operating-pnl, quantity-by-month | `YYYY-MM` | последние 5 полных месяцев |
| `date` | inventory/* | `YYYY-MM-DD` | конец прошлого месяца |
| `window` | inventory/* | целое 1..365 | `30` (дней) |
| `abc_class` | inventory/abc-turnover/detail | `A` \| `B` \| `C` \| `D` \| `—` (повторяемый) | — (все) |
| `article` | quantity-by-month | артикул (повторяемый, точное совпадение регистронезависимо) | — (все) |
| `search` | gross-profit, settlements, inventory/stock, inventory/abc-turnover/detail, quantity-by-month | строка ≤100 | — |
| `sort_by` | gross-profit, settlements, inventory/stock, inventory/abc-turnover/detail, quantity-by-month | см. ниже | `article` / `exposure` / `cost` / `total` |
| `sort_dir` | gross-profit, settlements, inventory/stock, inventory/abc-turnover/detail, quantity-by-month | `asc` \| `desc` | по полю (см. ниже) |
| `limit` / `offset` | gross-profit, settlements, inventory/stock, inventory/abc-turnover/detail, quantity-by-month | целые (limit 1..5000) | `100` / `0` |

**Пагинация.** Многострочные отчёты — `gross-profit`, `settlements`,
`inventory/stock`, `inventory/abc-turnover/detail`, `quantity-by-month` — поддерживают `?limit=&offset=` (как `/abc`, §4.1): ответ несёт
объект `pagination {total, limit, offset}`, `data[]` — запрошенная страница, а
`summary` считается по **полному** отчёту (исключение — `inventory/abc-turnover/detail`,
где `summary` считается по применённым фильтрам, см. «Фильтр по ABC-классу»). У
`gross-profit` с `search` поле `pagination.total` — число строк **после** фильтра,
`summary` — по всему отчёту.
...
**Поиск и сортировка.** `gross-profit`, `settlements`, `inventory/stock`,
`inventory/abc-turnover/detail` и `quantity-by-month` принимают
`?search=` и `?sort_by=…&sort_dir=asc|desc` (действуют и на `/export`). Применяются на
сервере **до** пагинации, поэтому `summary` по полному отчёту (кроме
`inventory/abc-turnover/detail`, где `summary` отражает фильтры), а
...
|---|---|---|---|
| `gross-profit` | артикул или наименование, подстрока | `article`\|`revenue`\|`gross_profit`\|`margin` (`article`) | `asc` для `article`, иначе `desc` |
| `settlements` | контрагент или ИНН, подстрока | `exposure`\|`receivable`\|`payable`\|`net` (`exposure`) | `desc` (`exposure = |ДЗ| + |КЗ|`) |
| `inventory/stock` | артикул или наименование, подстрока | `cost`\|`quantity`\|`days_no_movement`\|`turnover_days` (`cost`) | `desc` |
| `inventory/abc-turnover/detail` | артикул или наименование, подстрока | `cost`\|`quantity`\|`days_no_movement`\|`turnover_days`\|`margin` (`cost`) | `desc` |
| `quantity-by-month` | артикул или номенклатура, подстрока | `total`\|`article` (`total`) | `desc` (`asc` для `article`) |
...
**Фильтр по ABC-классу** (только `inventory/abc-turnover/detail`). `?abc_class=` сужает
строки до выбранного класса; несколько классов — повторением параметра
(`?abc_class=A&abc_class=D`). Допустимые значения — `A` `B` `C` `D` `—` (любое другое
→ `422`). **`summary` и строка ИТОГО в `/export` считаются по отфильтрованным строкам**
(фильтры `abc_class` + `search`): `rows_count` = число попавших под фильтр SKU и совпадает
с `pagination.total`, `quantity`/`cost_value` — суммы только по ним. Инвариант сохранности:
сумма `cost_value` по всем классам `A+B+C+D+—` равна полному итогу без фильтра. Подклассовые
суммы (`sku_count`+`cost_value` по каждому классу) также доступны из агрегированной матрицы
`inventory/abc-turnover`.
...
без артикула в 1С. Маркетплейсы исключены всегда (опт) — выбора канала нет.

**Карточки-дубли одного товара склеиваются по артикулу.** У одного варианта бывает
приписка «не использовать …» / «НЕ ИСПОЛЬЗОВАТЬ!!! …», у другого — нет; строки с одним
непустым артикулом суммируются в одну (в `name` — «чистое» имя без приписки, с наибольшей
выручкой). Безартикульные карточки остаются отдельными строками. Число объединённых
дублей — в `notes`.
...
**Считается с МП (все каналы)** — в отличие от прочих 1С-отчётов (без МП): расходы
90.07/90.08 даны по предприятию целиком и по каналам не делятся, поэтому и выручку берут
по всем каналам (иначе операционная прибыль уходит в искусственный минус). Об этом есть
приписка в `notes`. `lines[]` — иерархия (`level` = `section`|`line`): Продажи
(Выручка − Себестоимость) и Прочие расходы (Коммерческие 90.07.1 + Управленческие 90.08.1).
`summary` несёт `revenue / cogs / sales_net / selling / admin / other_expenses / total`.
ИТОГО **не** сходится с полным 1С-отчётом (нет сч. 91 и детализации по статьям).

#### `/onec/operating-pnl` — операционный P&L по месяцам

**Считается с МП (все каналы)**, как и `income-expense` (прочие 1С-отчёты — без МП; см.
приписку в `notes`). `data[]` — строка на месяц: `revenue, cogs, gross_profit,
selling_expenses, admin_expenses, operating_profit, margin_pct`. Месяцы без выгрузки
расходов оставляют `selling_expenses/admin_expenses/operating_profit/margin_pct = null`
(перечислены в `notes`). Ответ несёт `from_month` / `to_month` и `summary`.

#### `/onec/quantity-by-month` — количество по артикулам × месяцам

Сводная таблица (pivot): строки — артикулы, столбцы — месяцы диапазона, в ячейках —
количество проданного товара. Метрика — то же «Количество», что в
`/onec/gross-profit` (оборот по количеству, бух. сч. 90.01.1).

Зерно строки — **артикул** (размеры/характеристики суммируются). Карточки **без
артикула исключены** — это обобщённые ассортиментные/категорийные позиции 1С
(«Футболка муж/жен» и т.п.), а не конкретные товары. В продажах 1С они несут
бо́льшую часть количества, поэтому помесячный итог этого отчёта **меньше**, чем
`summary.quantity` у `gross-profit` (там безартикульные карточки идут
отдельными строками). Маркетплейсы исключены.

Ответ: `from_month` / `to_month`, `months[]` (упорядоченный список ключей-месяцев
`YYYY-MM` — порядок колонок/ось X графика), `summary {rows_count, total, by_month}`,
`pagination` и `data[]`. Строка `data[]` — `{article`, `name`, `by_month`
(`{"YYYY-MM": кол-во}` по всем месяцам, пустые = 0), `total}`. В `/export` (`.xlsx`)
каждый месяц — отдельная числовая колонка плюс колонка «Итого» и строка ИТОГО.

**Построение графика.** Ось X — `months[]`. Линия «всего» — `months.map(m =>
summary.by_month[m])`. Линии по артикулам — `data.map(d => ({label: d.article, data:
months.map(m => d.by_month[m] ?? 0)}))`.

**Выбор артикулов** (`?article=…` повторяемый) оставляет в `data[]` только выбранные
артикулы — набор рядов для графика. Применяется **после** итогов: `summary` (включая
`summary.by_month`) остаётся по полному отчёту (опорная линия «всего»), а
`pagination.total` — число выбранных. Тот же `article` действует на `/export`.

**`/onec/quantity-by-month/articles`** — источник для мультиселекта: артикулы периода
(`?from=&to=`, тот же набор, что и в отчёте) как `{from_month, to_month, articles:
[{article, name, total}], total}`, по убыванию количества. Значения `article` идут в
фильтр `?article=` отчёта.
...
`data[]` на дату среза: `article`, `name` (наименование номенклатуры, резолвится по
артикулу; может быть `""`), `quantity, cost_value`, `days_no_movement`,
`turnover_days` (остаток ÷ средний дневной расход за `window`). `cost_value` /
`turnover_days` / `days_no_movement` могут быть `null`. **`quantity` и `cost_value` —
бухгалтерский остаток сч.41 (`1C_account_balance_41`), консолидация по организациям
(Охана Маркет + Рея ТК)** — тот же источник, на котором строится отчёт «Запасы и
неликвиды» в самой 1С. сч.41 — срез на конец месяца, поэтому на дату не-конец-месяца
берётся ближайший доступный срез ≤ запрошенной даты (по умолчанию `?date=` = конец
прошлого месяца — попадает точно). Движение (`days_no_movement`, окно расхода) считается
из `1C_stock_turnover`. **Из набора SKU исключены артикулы, продававшиеся в месяце среза
только через МП** (есть продажа по сч.90 через маркетплейс, но нет ни одной не-МП) — отчёт
без МП; объём исключённого пишется в `notes`.

#### `/onec/inventory/abc-turnover` — матрица ABC × оборачиваемость

`data[]` — ячейки `abc_class × turnover_segment` со `sku_count` и `cost_value`.
ABC по марже месяца среза (без МП): A ≤80%, B ≤95%, C >95% кумул. положительной маржи; D —
маржа ≤ 0; «—» — нет продаж по сч.90. Сегменты: `<30 дн`, `30–60 дн`, `60–90 дн`, `90+ дн`,
`∞ (нет расхода)`, `нет остатка`. Артикулы, продававшиеся в месяце среза только через МП,
из отчёта исключены (см. `inventory/stock`) — иначе они засоряли бы класс «—»; объём
исключённого — в `notes`.
...
каждый со своими `abc_class` и `turnover_segment` плюс метрики запаса — `article, name,
...
(включая `margin`)/`?sort_dir=` и фильтр `?abc_class=`. `summary`
(`rows_count, quantity, cost_value`) и итог `/export` считаются по строкам, попавшим под
фильтры (`abc_class` + `search`): без фильтров — весь набор (совпадает с `inventory/stock`),
с фильтром — только отобранные SKU (`rows_count` = `pagination.total`).

#### Примеры

```bash
# Валовая прибыль (без МП) за апрель, первая страница
curl 'http://localhost:8000/api/v1/cfo/onec/gross-profit?month=2026-04&limit=100&offset=0'

# Поиск (подстрока по артикулу или наименованию) + сортировка по выручке + выгрузка Excel
curl 'http://localhost:8000/api/v1/cfo/onec/gross-profit?month=2026-04&search=халат'
curl 'http://localhost:8000/api/v1/cfo/onec/gross-profit?month=2026-04&sort_by=revenue&sort_dir=desc'
curl -OJ 'http://localhost:8000/api/v1/cfo/onec/gross-profit/export?month=2026-04'
...
# Количество по артикулам × месяцам (артикулы в строках, месяцы в столбцах) + Excel
curl 'http://localhost:8000/api/v1/cfo/onec/quantity-by-month?from=2026-01&to=2026-05'
curl 'http://localhost:8000/api/v1/cfo/onec/quantity-by-month?from=2026-01&to=2026-05&search=футболка&sort_by=total'
curl -OJ 'http://localhost:8000/api/v1/cfo/onec/quantity-by-month/export?from=2026-01&to=2026-05'
# Список артикулов для пикера + данные графика по выбранным (article повторяется)
curl 'http://localhost:8000/api/v1/cfo/onec/quantity-by-month/articles?from=2026-01&to=2026-05'
curl 'http://localhost:8000/api/v1/cfo/onec/quantity-by-month?from=2026-01&to=2026-05&article=VI-DT-4545&article=%D0%9F%D0%96'
...
| `/onec/operating-pnl`, `/onec/quantity-by-month` | `'from' and 'to' must be provided together` / `'from' must be <= 'to'` (формат `YYYY-MM`) |
...
| `orders_sum` | float | Сумма заказов, ₽ (ВБ: «Воронка продаж», как в ЛК; см. прим.) |
| `orders_qty` | int | Количество заказов (ВБ: «Воронка продаж», как в ЛК; см. прим.) |
| `buyouts_sum` | float | Сумма выкупов, ₽ (ВБ: «Воронка продаж»; см. прим.) |
| `buyouts_qty` | int | Количество выкупов (ВБ: «Воронка продаж»; см. прим.) |
...
> **Прим. (ВБ, «Заказано»/«Выкоплено»).** Для WB `orders_*`/`buyouts_*` берутся из отчёта
> «Воронка продаж» (analytics API), синкаемого AdolfDataSync — это тот же источник, что в
> личном кабинете, поэтому цифры сходятся с ЛК (июнь 2026: «Заказано» до 2 штук; авг–дек 2025:
> +0.04 %). Условие — период покрыт помесячными строками воронки: один календарный месяц или
> диапазон целых месяцев (суммируются). Неполный месяц (напр. 01–15 числа) или диапазон,
> кончающийся в середине месяца, → фолбэк на фид заказов (`wb_orders`/`wb_sales`), ~5–6 % ниже. При funnel-источнике `buyout_rate_pct`/`cogs_rub`/`margin_*` считаются
> от воронки. Ozon/YM не затронуты.
...
### Модели дашборда (`/dashboard/kpi`)

Определены в `cfo/services/dashboard_models.py`.

#### DashboardKpiReport

| Поле | Тип | Описание |
|------|-----|----------|
| `period` | PeriodOut | Выбранный период (`from`/`to`, включительно) |
| `granularity` | enum | Запрошенная разбивка рядов: `day` \| `month` \| `year` |
| `notes` | string[] | Методология и оговорки |
| `kpis` | DashboardKpi[] | 7 чисел в порядке `revenue, operating_profit, net_profit, cash_flow, money_on_accounts, receivable, net_margin` |

#### DashboardKpi

| Поле | Тип | Описание |
|------|-----|----------|
| `key` | string | Идентификатор числа (см. таблицу в §6.1.2) |
| `label` | string | Человекочитаемое название |
| `unit` | enum | `rub` \| `pct` |
| `value` | float \| null | Значение за выбранный период |
| `previous` | float \| null | То же за предыдущий эквивалентный период |
| `delta_pct` | float \| null | Относительный % к предыдущему периоду; для `net_margin` — процентные пункты; `null`, если сравнить не с чем |
| `series_granularity` | enum | Фактический грейн ряда (`day`\|`month`\|`year`); у `receivable` всегда `month` |
| `series` | DashboardKpiPoint[] | Ряд за выбранный период с разбивкой по `series_granularity` |

#### DashboardKpiPoint

| Поле | Тип | Описание |
|------|-----|----------|
| `period` | string | Ключ бакета: `YYYY-MM-DD` (day) / `YYYY-MM` (month) / `YYYY` (year) |
| `value` | float \| null | Значение метрики в этом бакете |
...
**OnecGrossProfitReport** — `{period: PeriodOut, notes, summary: OnecGrossProfitSummary, pagination: Pagination, data: OnecGrossProfitRow[]}`. Всегда без МП (опт) — параметра/поля канала нет.
...
| `quantity` | float | Остаток, шт — сальдо сч.41 (`quantity_balance`), конс. по организациям |
