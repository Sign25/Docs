...
| Cost Mapping   | Сопоставление выручки с себестоимостью из 1С:КА 2 (по артикулу) |
...
| ---------------------- | ------- | ------------------------------ |
| Выгрузка себестоимости | CSV     | Артикул (SKU) → COGS           |
| Бухгалтерская первичка | PDF/XLS | Накладные, счета-фактуры, акты |

<Note>
**Изменение v1.1:** Конфигурация 1С — Комплексная автоматизация 2 (1С:КА 2). Идентификация товаров производится по артикулу (SKU). Штрихкоды и номенклатурные группы не используются, т.к. не выгружаются из типовых отчётов 1С.
</Note>
